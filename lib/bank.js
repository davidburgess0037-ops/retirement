...
const seed = {
  ...
  balance: 570240,
  transactions: [
    // { id: "tx-7", label: "Transfer to Martha Ruth Canaday", date: "31 Jul, 2026", amount: -15000.0, type: "out", status: "Completed" },
...
export function getBankState() {
  ...
  try {
    ...
    // Seed was updated since this state was saved → refresh with new data.
    if (stored._v !== SEED_VERSION) {
      localStorage.setItem(KEY, JSON.stringify(seed));
      return seed;
    }
    return stored;
